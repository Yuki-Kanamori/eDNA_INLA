...
A part of catch statistics of small-scale bottom trawl fisheries recorded by several representative boats of Chiba Prefecture were provided by Chiba Prefecture. This data included date, geographic location, efforts (number of tows), gear, and catch weight (kg) in each fish. Almost of all gear was beam trawl although dredge net also used. The species which also detected by eDNA was \textit{Conger myriaster} (マアナゴ), \textit{Kareius bicoloratus} (イシガレイ), \textit{Pseudopleuronectes yokohamae} (マコガレイ), \textit{Lateolabrax japonicus} (スズキ), \textit{Konosirus punctatus} (コノシロ), \textit{Acanthopagrus schlegelii}, species group of \textit{Pennahia} which includes \textit{Pennahia argentata} and \textit{Nibea mitsukurii} (イシモチ類), and species group of \textit{Sphyraena} which includes \textit{Sphyraena pinguis}, \textit{Sphyraena japonica}, and \textit{Promethichthys prometheus}. Thus, we estimated the spatial distribution of these four species using the eDNA-IDM. \textcolor{red}{カマス類とイシモチ類の種群の書き方を工夫する．あと，表でまとめる？}
