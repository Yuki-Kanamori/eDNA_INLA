...
Hence, if we develop a IDM that uses not only eDNA data but also the second data which contains the information that species exists at a location, as well as treating environmental factors as covariance in the model, then the spatial uncertainty of eDNA should be reduced and the model inference should be improved.
...
to infer spatial distribution of species from eDNA, we first make a spatial distribution model which considers spatial uncertainty of eDNA by using an integrated spatial distribution model. 
