...
However, the keep for all focal species in laboratory is not realistic and low cost-efficiency when multi species studies such as biodiversity monitoring and fisheries management. Moreover, biologists monitoring species are often not familiar with hydrological models and the simulation of eDNA often seems to be hard for them.
...
Hence, if we develop a IDM that uses not only eDNA data but also the second data which contains the information that species exists at a location, then the inference of the spatial distribution form the model should be reduced about spatial uncertainty of eDNA and should be improved.   
...
\ \ \ \ \ \ \ \ \ \ 
If we use one or more data that include the information about species existence in addition to eDNA data and infer the spatial distribution with combing these data by using IDMs, the spatial uncertainty of eDNA should recuse and the accuracy of prediction should increase. 

統合モデルを使えば，eDNAデータだけで空間分布を推定するよりも推定精度が上がり，
In this paper, 
...
to infer spatial distribution of species from eDNA, we first make a spatial distribution model wichi considers spatial uncertainty of eDNA by using an integrated spatial distribution model. 
We then apply the model to both eDNA data and catch statistics for four Edomae fish in Tokyo Bay, Japan. 
