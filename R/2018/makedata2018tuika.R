...
# イシモチ類のやり直し ----------------------------------------------------
dir_input = "/Users/Yuki/Dropbox/TokyoBay_CPUE/nominalCPUE/"
dir_output = "/Users/Yuki/Dropbox/eDNA_INLA/"
...
setwd(dir = dir_input)
path = dir_input
files = list.files(path, pattern = "ｲｼﾓﾁ類")
...
jap_cog <- jap[jap$lat > 35 & jap$lat < 38 & jap$long > 139 & jap$long < 141, ]
t2 <- p + geom_polygon(data = jap_cog, aes(x=long, y=lat, group=group), colour="gray 50", fill="gray 50")+ coord_map(xlim = c(139.5, 140.3), ylim = c(35, 35.75))
t2 + geom_point(data = y2018, aes(x = lon, y = lat), shape = 16, size = 1)
summary(y2018)
...
setwd(dir = dir_output)
all = read.csv("joint_cpue2018_tuikasp.csv")
colnames(all)
all = rbind(all %>% filter(sp != "isimoti-rui") %>% select(-X), `isimoti-rui`)

write.csv(all, "joint_cpue2018_tuikasp2.csv")
