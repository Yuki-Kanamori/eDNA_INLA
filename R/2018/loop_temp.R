...
for(i in 1:length(splist)){
  ...
  e_stk = inla.stack(data = list(y = cbind(edna, NA)),
                     A = list(e_A, 1),
                     effects = list(list(i.e = 1:mesh2$n, i.e2 = 1:mesh2$n), list(eb.0 = rep(1, length(edna)), temp = scale(e_fish$temp))),
                     tag = "e_dat")
  ...
  ep_stk = inla.stack(data = list(y = cbind(na[, 1], na[, 2])),
                      ...
                      effects = list(list(i.e = 1:mesh2$n, i.e2 = 1:mesh2$n), 
                                     list(eb.0 = rep(1, nrow(coop)), temp = rep(1, nrow(coop)))),
                      tag = "ep_dat")
  ...
  dir_save = paste0(dirname, "/", Sys.Date(), "temp")
  dir.create(dir_save)
  setwd(dir = dir_save)
