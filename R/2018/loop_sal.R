...
for(i in 1:length(splist)){
  ...
  effect = rbind(data.frame(x = best_kono$summary.random$salinity$ID, y = best_kono$summary.random$salinity$mean, variable = "Sal"))
  effect = effect %>% filter(x != 1)
  effect$variable = factor(effect$variable, levels = c("Sal"))
