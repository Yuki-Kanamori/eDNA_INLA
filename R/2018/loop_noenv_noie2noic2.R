...
for(i in 1:length(splist)){
  ...
  c_stk = inla.stack(data = list(y = cbind(NA, catch)),
                     A = list(c_A, 1),
                     effects = list(list(i.c = 1:mesh2$n), list(cb.0 = rep(1, length(catch)))),
                     tag = "c_dat")
  cp_stk = inla.stack(data = list(y = cbind(na[, 1], na[, 2])),
                      A = list(Ap, 1),
                      effects = list(list(i.c = 1:mesh2$n), 
  ...
  dir_save = paste0(dirname, "/", Sys.Date(), "noenv_Nie2Nic2")
  dir.create(dir_save)
  setwd(dir = dir_save)
