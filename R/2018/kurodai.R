...
# クロダイ ------------------------------------
e_fish = mifish %>% filter(sp == "kurodai", layer == "B")
summary(e_fish)
...
e_stk = inla.stack(data = list(y = cbind(edna, NA)),
                   A = list(e_A, 1),
                   effects = list(list(i.e = 1:mesh2$n, i.e2 = 1:mesh2$n), list(eb.0 = rep(1, length(edna)), temp = scale(e_fish$temp), salinity = scale(e_fish$salinity), pH = scale(e_fish$pH))),
                   tag = "e_dat")
...
c = coord_fixed(ratio = 1)
s = scale_fill_gradient(name = "encounter prob. (logit)", low = "blue", high = "orange")
g+t+f+c+s+pol+c_map+theme_bw()+labs(title = "kurodai")
...
c = coord_fixed(ratio = 1)
s = scale_fill_gradient(name = "encounter prob. (logit)", low = "blue", high = "orange")
g+t+c+s+pol+c_map+theme_bw()+labs(title = "kurodai")
