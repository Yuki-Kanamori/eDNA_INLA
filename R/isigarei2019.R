...
# 2019 ----------------------------------------------------------
# eDNA & env ----------------------------------------------------------
mifish = read.csv("joint_edna2019.csv")

# sp1 = read.xlsx("splist2019.xlsx", sheet = 1, colNames = T)
# sp2 = read.xlsx("splist2019.xlsx", sheet = 2, colNames = T)

# catch ---------------------------------------------------------
# データは1-12月
data = read.csv("joint_cpue2019.csv")


# スズキ ------------------------------------
e_fish = mifish %>% filter(sp_group == "isigarei", SorB == "B")
summary(e_fish)
unique(e_fish$ID)
edna = (e_fish$copy > 0) + 0

c_fish = data %>% filter(sp == "isigarei")
summary(c_fish)
...
#                    A = list(e_A, 1),
#                    effects = list(list(i.e = 1:mesh2$n, i.e2 = 1:mesh2$n), list(eb.0 = rep(1, length(edna)), temp = e_fish$temp, salinity = e_fish$salinity, DO = e_fish$DO_mg, pH = e_fish$pH)),
#                    tag = "e_dat")
...
e_stk = inla.stack(data = list(y = cbind(edna, NA)),
                   A = list(e_A, 1),
                   effects = list(list(i.e = 1:mesh2$n, i.e2 = 1:mesh2$n), list(eb.0 = rep(1, length(edna)), temp = e_fish$temp, salinity = e_fish$salinity, pH = e_fish$pH)),
                   tag = "e_dat")
na = as.matrix(cbind(rep(NA, nrow(coop)), rep(NA, nrow(coop))))
ep_stk = inla.stack(data = list(y = cbind(na[, 1], na[, 2])),
                    A = list(Ap, 1),
...
# *** ERROR *** 	fail to create directory [/private/var/folders/9g/2v4y6m0x5vs6ccnjwtdf6py40000gp/T/RtmpTp9dhR/file2b33361f6d7/results.files/random.effect00000001]: No such file or directory
# 
# sh: line 1: 15503 Segmentation fault: 11  '/Library/Frameworks/R.framework/Versions/4.0/Resources/library/INLA/bin/mac/64bit/inla.run' -b -s -v -t 4 '/private/var/folders/9g/2v4y6m0x5vs6ccnjwtdf6py40000gp/T/RtmpTp9dhR/file2b33361f6d7/Model.ini' > '/private/var/folders/9g/2v4y6m0x5vs6ccnjwtdf6py40000gp/T/RtmpTp9dhR/file2b33361f6d7/Logfile.txt'
# inla.inlaprogram.has.crashed() でエラー: 
#   The inla-program exited with an error. Unless you interupted it yourself, please rerun with verbose=TRUE and check the output carefully.
# If this does not help, please contact the developers at <help@r-inla.org>.

res_mako$waic$waic; res_mako$dic$dic #929.9, 928.4
...
c = coord_fixed(ratio = 1)
s = scale_fill_gradient(name = "encounter prob. (logit)", low = "blue", high = "orange")
g+t+c+s+pol+c_map+theme_bw()+labs(title = "suzuki")
#pred_mean_catch
...
mean_s_ie = inla.mesh.project(proj_e, best_kono$summary.random$i.e$mean) #i.c??
sd_s_ie = inla.mesh.project(proj_e, best_kono$summary.random$i.e$sd) #i.c??
...
effect$variable = factor(effect$variable, levels = c("temp", "sal", "ph"))
