...
# マアナゴ ------------------------------------
e_fish = mifish %>% filter(sp_group == "maanago", SorB == "B")
summary(e_fish)
unique(e_fish$ID)
edna = (e_fish$copy > 0) + 0

c_fish = data %>% filter(sp == "maanago")
summary(c_fish)
...
res_ana$waic$waic; res_ana$dic$dic #11606, 11582
...
c = coord_fixed(ratio = 1)
s = scale_fill_gradient(name = "encounter prob. (logit)", low = "blue", high = "orange")
g+t+c+s+pol+c_map+theme_bw()+labs(title = "maanago")
#pred_mean_catch
...
labs = labs(x = "Environmental variable", y = "Effect of environment", title = "maanago")
g+l+f+labs+theme_bw()
