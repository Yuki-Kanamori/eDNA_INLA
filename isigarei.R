...
# コノシロ ------------------------------------
e_fish = mifish %>% filter(sp == "isigarei", layer == "B")
summary(e_fish)
edna = (e_fish$count > 0) + 0

c_fish = data %>% filter(FISH == "isigarei")
...
res_isi$waic$waic; res_isi$dic$dic #640.3, 640.4
summary(res_isi)
...
pred_mean_e = best_isi$summary.fitted.values[index_ep, "mean"]
pred_mean_c = best_isi$summary.fitted.values[index_cp, "mean"]
pred_ll_e = best_isi$summary.fitted.values[index_ep, "0.025quant"]
pred_ul_e = best_isi$summary.fitted.values[index_ep, "0.975quant"]
pred_ll_c = best_isi$summary.fitted.values[index_cp, "0.025quant"]
pred_ul_c = best_isi$summary.fitted.values[index_cp, "0.975quant"]
...
c = coord_fixed(ratio = 1)
s = scale_fill_gradient(name = "encounter prob. (logit)", low = "blue", high = "orange")
g+t+f+c+s+pol+c_map+theme_bw()+labs(title = "isigarei")
...
mean_s_ie = inla.mesh.project(proj_e, best_isi$summary.random$i.e$mean)
sd_s_ie = inla.mesh.project(proj_e, best_isi$summary.random$i.e$sd)
...
# range_e = apply(coop, 2, range)
proj_e = inla.mesh.projector(mesh2, xlim = range_e[, 1], ylim = range_e[, 2], dims = c(50, 50))
mean_s_ic2 = inla.mesh.project(proj_e, best_isi$summary.random$i.c2$mean)
sd_s_ic2 = inla.mesh.project(proj_e, best_isi$summary.random$i.c2$sd)
...
# range_e = apply(coop, 2, range)
proj_e = inla.mesh.projector(mesh2, xlim = range_e[, 1], ylim = range_e[, 2], dims = c(50, 50))
mean_s_ie2 = inla.mesh.project(proj_e, best_isi$summary.random$i.e2$mean)
sd_s_ie2 = inla.mesh.project(proj_e, best_isi$summary.random$i.e2$sd)
...
effect = rbind(data.frame(x = best_isi$summary.random$temp$ID, y = best_isi$summary.random$temp$mean, variable = "temp"),
               data.frame(x = best_isi$summary.random$salinity$ID, y = best_isi$summary.random$salinity$mean, variable = "sal"),
               data.frame(x = best_isi$summary.random$DO$ID, y = best_isi$summary.random$DO$mean, variable = "do"),
               data.frame(x = best_isi$summary.random$pH$ID, y = best_isi$summary.random$pH$mean, variable = "ph"))
